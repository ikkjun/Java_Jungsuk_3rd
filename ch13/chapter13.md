...
##### wait( )
객체의 lock을 풀고 쓰레드를 해당 객체의 waiting pool(대기실)에 넣는다.
##### notify( )
해당 객체의 waiting pool에 있던 모든 쓰레드 중 임의의 쓰레드 하나를 깨운다.
##### notifyAll( )
waiting pool에서 대기중인 모든 쓰레드에게 통보를 하지만, 그래도 lock을 얻을 수 있는 것은 하나의 쓰레드일 뿐이고 나머지 스레드는 통보를 받긴 했지만, lock을 얻지 못하면 다시 lock을 기다리는 신세가 된다.
