...
배열(ArrayList, Vector의)의 장단점
장점: 구조 간단, 데이터를 읽어오는데 걸리는 시간(접근 시간)이 가장 빠르다.
단점1: 크기를 변경할 수 없다. (① 더 큰 배열을 생성, ② 복사, ③ 참조 변경)
단점2: 비순차적인 데이터의 추가 또는 삭제에 시간이 많이 걸린다. 
(데이터를 추가하거나 삭제하기 위해 다른 데이터를 옮겨야 하기 때문이다. 
그러나 순차적인(끝에) 데이터 추가와 삭제는 빠르다.)
...
더블리 링크드 리스트(doubly linked list) - 자바에서 구현
...
느림
비고
순차적인 추가, 삭제는 빠르나 비효율적으로 메모리를 사용한다.
데이터가 많을수록 접근성이 떨어진다.
