...
HashMap은 Map인터페이스를 구현했으므로 키와 값을 묶어서 하나의 데이터(entry)로 저장한다는 특징을 갖는다.</br> 
...
서로 다른 키일지라도 같은 값의 해시코드를 반환할 수도 있다.</br>
...
| HashMap. HashTable           |배열과 연결이 결합된 형태. 추가, 삭제, 검색, 접근성이 모두 뛰어남. 검색에는 최고성능을 보인다.|
